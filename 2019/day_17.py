...
program_input = open("input_17.txt").readline()
...
for char in robot:
    if char == 10:
        ...
        board.append([])
        continue
    board[-1].append(0 if chr(char) == "." else 1)
    x += 1
board = board[:-2]
total = 0
for x in range(len(board[0])):
    for y in range(len(board)):
        if 0 < y < len(board) - 1 and 0 < x < len(board[0]) - 1:
            if board[y][x] and board[y][x - 1] and board[y][x + 1] and board[y - 1][x] and board[y + 1][x]:
                total += x * y
print(total)
